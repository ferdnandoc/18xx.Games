  ...
  module Game
    class GameInfo < Snabberb::Component
      ...
      def render_body
        ...
        children.concat(extra_status_panel) if @game.extra_status_panel
        children.concat(endgame)
        children << h(GameMeta, game: @game)
      end

      def extra_status_panel
        panel = @game.extra_status_panel

        cell_props = { style: { textAlign: 'center', minWidth: '2.6rem', padding: '0.2rem' } }

        rows = panel[:rows].map do |row|
          h(:tr, row.map { |cell| h(:td, cell_props, cell) })
        end

        children = [
          h(:h3, panel[:title]),
          h(:div, { style: { overflowX: 'auto' } }, [h(:table, [h(:tbody, rows)])]),
        ]
        children << h(:p, panel[:footnote]) if panel[:footnote]
        children
      end
